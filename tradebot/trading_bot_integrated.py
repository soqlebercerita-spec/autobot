...
class TradingBot:
    ...
    def create_gui_components(self):
        ...
        tpsl_frame.pack(side="right", fill="y", padx=10)
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    if len(prices) < 50:
                        self.safe_log("⚠️ Insufficient market data", "WARNING")
                        time.sleep(scan_interval)
                        continue
                    
                    # Enhanced signal analysis
                    signal_data = self.indicators.enhanced_signal_analysis(prices, symbol)
                    
                    # Determine trading thresholds
                    if self.hft_mode_var.get():
                        confidence_threshold = config.SIGNAL_CONFIDENCE_THRESHOLD_HFT
                    ...
                    if (signal_data['signal'] in ['BUY', 'SELL'] and 
                        signal_data['confidence'] >= confidence_threshold and
                        signal_data['strength'] >= strength_threshold):
                        
                        # Execute trade
                        if self.execute_enhanced_trade(symbol, signal_data):
                            self.total_opportunities_captured += 1
                            self.safe_log(f"🎯 Trade executed: {signal_data['signal']} {symbol}", "TRADE")
                            self.safe_log(f"   Confidence: {signal_data['confidence']:.2f}", "INFO")
                            self.safe_log(f"   Reasons: {', '.join(signal_data['reasons'])}", "INFO")
                        else:
                            self.total_opportunities_missed += 1
                            self.safe_log("❌ Trade execution failed", "ERROR")
                    else:
                        if signal_data['signal'] != 'HOLD':
                            self.safe_log(f"📈 Signal: {signal_data['signal']} | "
                                        f"Confidence: {signal_data['confidence']:.2f} | "
                                        f"Strength: {signal_data['strength']:.2f} (Below threshold)", "INFO")
                        else:
                            self.safe_log("📊 Market analysis: HOLD position", "INFO")
            ...
            self.safe_log("🛑 Trading loop stopped", "INFO")
    ...
    def execute_enhanced_trade(self, symbol, signal_data):
        ...
        try:
            ...
            # Auto TP/SL calculation based on balance
            if self.auto_tp_sl_var.get():
                if self.hft_mode_var.get():
                ...
                tp_amount = balance * tp_percent
                sl_amount = balance * sl_percent
            else:
                tp_amount = 100  # Default
                sl_amount = 50   # Default
            ...
            if signal_data['signal'] == 'BUY':
                tp_price = current_price + (tp_amount / (lot_size * 100000))
                sl_price = current_price - (sl_amount / (lot_size * 100000))
            ...
            if MT5_AVAILABLE and self.connected:
                ...
                if is_profitable:
                    profit = tp_amount * random.uniform(0.7, 1.0)
                    self.session_profit += profit
                    self.safe_log(f"✅ Simulated trade PROFIT: ${profit:.2f}", "SUCCESS")
                else:
                    loss = -sl_amount * random.uniform(0.7, 1.0)
                ...
                self.safe_log(f"   Type: {signal_data['signal']}", "INFO")
                self.safe_log(f"   Price: {execution_price:.5f}", "INFO")
                self.safe_log(f"   TP Target: ${tp_amount:.2f} | SL Risk: ${sl_amount:.2f}", "INFO")
